...
public class SociosDAO{
    Connection conexion;
         ...
         public Socios mostrarSocio(int _id){
             ...
             while(rs.next()){
                 ...
                 String telefono=rs.getString("telefono");
                 boolean activo=rs.getBoolean("activo");                 
                 s1=new Socios(id,nombre,apellido,direccion,localidad,fechaNac,email,telefono,activo);
             }
         ...
         public boolean InsertSocios(Socios s1){
             ...
             try{
                 ...
                 ps.setObject(5, s1.getFechaNac());
                 ps.setString(6, s1.getEmail());
                 ps.setString(7, s1.getTelefono());
                 ps.setBoolean(8, true);
                 ps.executeUpdate();
                 JOptionPane.showMessageDialog(null,"Se añadío un nuevo socio correctamente");
    ...
    public boolean ActualizarSocios(Socios s1){
        PreparedStatement ps;        
        try{
            ps=conexion.prepareStatement("update socios set nombre=?,apellido=?,direccion=?,localidad=?,fechaNac=?,email=?,telefono=?,activo=? where idSocio=?");
            ps.setString(1, s1.getNombre());
            ps.setString(2, s1.getApellido());
            ps.setString(3, s1.getDireccion());
            ps.setString(4, s1.getLocalidad());
